...
- `blog/assets/` — hero images (`<slug>.jpg`). No image → article renders without a hero (cards fall back to text).
  **Covers are generated by Claude Code (standing rule, 2026-09-14)** with `~/.local/bin/iomi-blog-cover.py --prompt "<custom>"` (fal.ai Flux; key in `~/.local/share/fal-auth`, never printed). House style = the existing series: cinematic photorealistic scene, a founder/professional facing a mint-green holographic dashboard, night skyline bokeh, no text/logos/readable numbers, 16:9. Every new post gets its cover generated (and pasted into the vault `assets/` + `hero_image:` frontmatter) before or at publication — no cover-less posts.
